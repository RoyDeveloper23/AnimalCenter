...
import type { Appointment, HistoryRecord, Pet } from "@/types";
...
export function DoctorDashboard() {
  ...
  const past = myAppointments.filter(
    (a) => a.status === "attended" || a.status === "no-show",
  );
  ...
  return (
    <div className="min-h-screen bg-stone-50 paw-bg-subtle">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-stone-100 mb-6">
          <div className="flex items-center gap-4">
            <div
              className={`w-14 h-14 ${myArea === "spa" ? "bg-purple-500" : "bg-green-500"} rounded-2xl flex items-center justify-center shadow-md`}
            >
              {myArea === "spa" ? (
            ...
            <div>
              ...
              <span
                className={`badge ${myArea === "spa" ? "badge-purple" : "badge-green"}`}
              >
                {areaLabel}
              </span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            {
              label: "Atendidos",
              value: past.filter((a) => a.status === "attended").length,
              color: "green",
            },
            {
              label: "No asistieron",
              value: past.filter((a) => a.status === "no-show").length,
              color: "red",
            },
          ].map((s) => (
            <div
            ...
            {pending.length === 0 && past.length === 0 && (
              <div className="bg-white rounded-2xl border-2 border-dashed border-stone-200 p-12 text-center">
                ...
                <p className="text-stone-500">
                  No hay citas asignadas para esta área
                </p>
              </div>
            )}
            {pending.length > 0 && (
              <div>
                <h3 className="font-black text-stone-900 mb-3">
                  Próximas citas
                </h3>
                ...
                <div className="space-y-3">
                  {past.map((apt) => {
                    ...
                    return (
                      <div
                        key={apt.id}
                        className="bg-white rounded-2xl p-4 border border-stone-100 shadow-sm flex items-center justify-between"
                      >
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`badge ${apt.status === "attended" ? "badge-green" : "badge-red"}`}
                          >
                            {apt.status === "attended"
                              ? "Atendido"
                              : "No asistió"}
        ...
        {tab === "historial" && (
          <div className="space-y-4">
            <h2 className="text-lg font-black text-stone-900">
              Historial del área: {areaLabel}
            </h2>
            {history.filter((h) => h.area === myArea).length === 0 ? (
              <div className="bg-white rounded-2xl border-2 border-dashed border-stone-200 p-12 text-center">
                <p className="text-4xl mb-3">📋</p>
                <p className="text-stone-500">
                  Aún no hay registros en esta área
                </p>
              </div>
            ) : (
              [...history]
      ...
      {selectedPet && (
        <PetProfileModal
          pet={selectedPet}
          petHistory={history.filter((h) => h.petId === selectedPet.id)}
    ...
    <div className="bg-white rounded-2xl p-6 border border-stone-100 shadow-sm">
      ...
      {linkedApt && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-5 flex items-center justify-between">
          <p className="text-sm text-green-700 font-semibold">
            ✅ Vinculado a cita confirmada de {linkedApt.petName}
          </p>
          <button
            onClick={onClearApt}
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            value={myArea === "spa" ? "SPA" : "Clínica Veterinaria"}
            disabled
            className="input-field bg-stone-50 text-stone-500 cursor-not-allowed"
          />
        </div>
        <div>
          ...
          {linkedApt ? (
            <input
              type="text"
              value={`${selectedPet?.avatar ?? ""} ${selectedPet?.name ?? linkedApt.petName}`}
              disabled
              className="input-field bg-stone-50 text-stone-500 cursor-not-allowed"
            />
          ) : (
          ...
          {selectedPet?.allergies && (
            <p className="text-xs text-amber-600 mt-1">
              ⚠️ Alergias: {selectedPet.allergies}
            </p>
          )}
        </div>
        <div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={
              myArea === "spa"
                ? "Ej: Baño de aromaterapia con lavanda. Corte de uñas. Se portó muy bien..."
                : "Ej: Vacuna antirrábica aplicada. Peso: 18kg. Temperatura normal. Próxima cita en 6 meses..."
            }
          />
        </div>
        <button
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-lg max-h-[85vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <h4 className="font-black text-stone-900 mb-3">
              Historial completo
            </h4>
            {petHistory.length === 0 ? (
                ...
                {[...petHistory].reverse().map((h) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`badge ${h.area === "spa" ? "badge-purple" : "badge-green"} text-[10px]`}
                      >
